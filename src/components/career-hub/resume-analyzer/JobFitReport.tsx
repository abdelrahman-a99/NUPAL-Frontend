      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 px-2">
        <div className="flex items-center gap-5">
          <div>
            ...
            <p className="text-sm text-slate-500 font-bold uppercase tracking-wider opacity-60">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-8 space-y-6">
          ...
          {data.redFlags && data.redFlags.length > 0 && (
            <section className="bg-red-50 p-6 rounded-2xl border border-red-100 shadow-sm animate-pulse-subtle">
              <div className="flex items-center gap-3 mb-4 text-red-600">
                <div className="p-2 bg-red-100 rounded-lg"><AlertCircle className="w-5 h-5" /></div>
                <h2 className="text-sm font-black uppercase tracking-widest">Critical Concerns (Red Flags)</h2>
              </div>
          ...
          <section className="bg-white p-8 rounded-2xl border border-slate-200 shadow-sm">
            <div className="flex items-center gap-3 mb-8">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400 font-semibold mt-0.5">Personalized action plan based on your profile</p>
              </div>
            </div>
            <div className="space-y-5">
              {data.recommendations.map((rec, i) => {
                // Extract first sentence as a card title
                const firstDot = rec.search(/[.!?]/);
                const title = firstDot > 0 && firstDot < 120 ? rec.slice(0, firstDot + 1) : rec.slice(0, 80) + '…';
                const body = firstDot > 0 && firstDot < 120 ? rec.slice(firstDot + 1).trim() : rec;
                return (
                  <div key={i} className="group flex gap-5 bg-slate-50/60 hover:bg-blue-50/40 border border-slate-100 hover:border-blue-100 rounded-2xl p-5 transition-all duration-200">
                    {/* Step number */}
                    <div className="shrink-0 w-9 h-9 rounded-xl bg-blue-600 text-white flex items-center justify-center font-black text-sm shadow-sm">
                      {i + 1}
                    </div>
                    <div className="space-y-2">
                      <p className="text-[13px] font-black text-slate-900 leading-snug">{title}</p>
                      {body && body.length > 5 && (
